...
export const FortniteCart: React.FC<FortniteCartProps> = ({ isOpen, onClose, user, adminUsername, adminPlatform }) => {
  ...
  const handleCheckout = () => {
    ...
    if (!currentUser) {
      displayToast('Debes iniciar sesion para realizar una compra');
      return;
    }
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-[#0D0B1E] z-[501] overflow-y-auto"
            >
              {!showCheckout ? (
                <div className="p-6">
                  ...
                  {cart.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-white/40">Tu carrito esta vacio</p>
                    </div>
                  ) : (
                    <>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-white/60 text-sm mb-2">Informacion de contacto</label>
                      <input
                        type="text"
                        value={contactInfo}
                        onChange={(e) => setContactInfo(e.target.value)}
                        placeholder="Email, Discord o Numero"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:border-blue-500/50"
                      />
                    </div>
